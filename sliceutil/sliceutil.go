// Package sliceutil provides generic, type-safe utility functions for working with slices,
...
func Map[S ~[]E, E any, R any](collection S, iteratee func(item E, index int) R) []R {
	if collection == nil {
		return nil
	}
...
func Pluck[S ~[]E, E any, R any](collection S, propertyGetter func(item E) R) []R {
	if collection == nil {
		return nil
	}
	result := make([]R, len(collection))
...
func Reduce[S ~[]E, E any, R any](collection S, initialValue R, reducer func(acc R, item E, index int) R) R {
	if len(collection) == 0 {
		return initialValue
	}
