...
func TestMapReduce(t *testing.T) {
	t.Run("sums squares of integers", func(t *testing.T) {
		...
		result := MapReduce(
			input,
			func(item int, index int) int { return item * item },
			0,
			func(acc int, mapped int, index int) int { return acc + mapped },
		)
		if result != expected {
	...
	t.Run("builds a comma-separated string", func(t *testing.T) {
		...
		result := MapReduce(
			input,
			func(item string, index int) string { return strings.ToUpper(item) },
			"",
	...
	t.Run("returns initial value for empty input", func(t *testing.T) {
		...
		result := MapReduce(
			input,
			func(item int, index int) int { return item },
			10,
			func(acc int, mapped int, index int) int { return acc + mapped },
		)
		if result != expected {
...
func TestFindFirst(t *testing.T) {
	t.Run("finds first even number", func(t *testing.T) {
		...
		expected := 4
		result, found := FindFirst(input, func(item int, index int) bool {
			return item%2 == 0
		})
	...
	t.Run("returns false when no match", func(t *testing.T) {
		input := []int{1, 3, 5, 7, 9}
		_, found := FindFirst(input, func(item int, index int) bool {
			return item%2 == 0
		})
		if found {
...
func TestFindLast(t *testing.T) {
	t.Run("finds last even number", func(t *testing.T) {
		...
		expected := 6
		result, found := FindLast(input, func(item int, index int) bool {
			return item%2 == 0
		})
		if !found || result != expected {
	...
	t.Run("returns false when no match", func(t *testing.T) {
		input := []int{1, 3, 5, 7, 9}
		_, found := FindLast(input, func(item int, index int) bool {
			return item%2 == 0
		})
		if found {
...
func TestPartition(t *testing.T) {
	t.Run("partitions even and odd numbers", func(t *testing.T) {
		...
		even, odd := Partition(input, func(item int, index int) bool {
			return item%2 == 0
		})
